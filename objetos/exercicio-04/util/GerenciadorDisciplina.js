...
function menuDisciplinas() {
    return scanner.questionInt("Qual disciplina você gostaria de cadastrar\n" +
        "1 - Disciplina Normal\n" +
...
function criarDisciplina() {
    ...
    if (opcao === 1) {
        ...
        const disciplinaPratica = new DisciplinaPratica(cargaHorariaPratica, codigo, nome, cargaHoraria);
...
function cadastrarDisciplina(disciplinas) {
    ...
    if (buscarDisciplina(disciplinas, disciplina.codigo) === undefined) {
        ...
        console.log("Erro ao cadastrar disciplina! O código informado já foi cadastrado!");
    }
