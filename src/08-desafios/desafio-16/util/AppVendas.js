  ...
  do {
    ...
    if (produto.estoqueAtual > 0) {
      ...
      while (qtdProduto > produto.estoqueAtual) {
        qtdProduto = scanner.questionInt(
          "Quantidade superior ao estoque. Digite outro valor: "
        );
      }
    ...
    novoProduto = scanner.questionInt(
      "Adicionar novo produto? (1 - sim/2 - não) "
    );
    while (novoProduto !== 1 && novoProduto !== 2) {
      novoProduto = scanner.questionInt(
        "Opção inválida! Adicionar novo produto? (1 - sim/2 - não) "
      );
    }
  } while (novoProduto === 1);
}
...
function mostrarNotaFisca(venda) {
  console.log(venda.toString() + "Produtos");
