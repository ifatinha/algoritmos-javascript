/** Capitulo 08 - sub-rotinas: Exemplo 11
...
function lerDados() {
  for (let i = 1; i <= 5; i++) {
    ...
    let olhos = scanner.question(
      "Cor dos Olhos (A - azul; C - Castanho; P - Preto; V - Verde): "
    );
    while (verificarDados(dadosOlhos, olhos.toLowerCase()) === false) {
      olhos = scanner.question(
        "Opção inválida! Cor dos Olhos (A - azul; C - Castanho; P - Preto; V - Verde): "
      );
    }
    corOlhos.push(olhos);

    let cabelo = scanner.question(
      "Cor dos Cabelos (C - Castanho; L - Louros; P - Preto; R - Ruivo): "
    );
    while (verificarDados(dadosCabelo, cabelo.toLowerCase()) === false) {
      cabelo = scanner.question(
        "Opção inválida! Cor dos Cabelos (C - Castanho; L - Louros; P - Preto; R - Ruivo): "
      );
    }
    ...
    while (verificarDados(dadosSexo, sexo.toLowerCase()) === false) {
      sexo = scanner.question(
        "Opção inválida! Sexo (F - feminino, M - masculino): "
      );
    }
    sexo.push(cabelo);
...
lerDados();
console.log(
  "Média de idade das pessoas que possuem Olhos Castanhos e Cabelos Pretos: " +
    mediaIdade()
);
console.log("A maior idade entre as idades digitadas foi " + maiorIdade());
console.log(
  "A quantidade de mulheres com idade entre 18 e 35 anos, que possuem cabelos loures e olhos azuis " +
    "foi de: " +
    qtdMulheres()
);
