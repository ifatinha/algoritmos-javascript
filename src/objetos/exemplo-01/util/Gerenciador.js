...
export function registerProduct(types) {
  ...
  let typeCode = scanner.questionInt(
    "Digite o tipo do produto: (1 - Limpeza / 2 - Alimentação): "
  );

  while (typeCode !== types[0]._code && typeCode !== types[1]._code) {
    typeCode = scanner.questionInt(
      "Código inválida! Digite o tipo do produto novamente: (1 - Limpeza / 2 - Alimentação): "
    );
  }
