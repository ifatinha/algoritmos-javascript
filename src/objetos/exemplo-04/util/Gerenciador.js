  ...
  imoveis.forEach(function (imovel) {
    console.log(
      "Proprietário: " +
        ...
        imovel.porcentagemComissao +
        "%\n" +
        "Valor Comissão de Venda: R$" +
        imovel.valorComissao +
        "\n" +
        "IPTU: R$" +
        imovel.calcularIPTU().toFixed(2) +
        "\n\n"
    );
  });
}
...
function alterarSituacaoImovel(index, imoveis) {
  if (index >= 0 && index < imoveis.length) {
    let novaSituacao = scanner.questionInt(
      "A situação do imóvel é " +
        imoveis[index].situacao +
        "\n" +
    ...
    if (novaSituacao === 2) {
      let porcentagem = scanner.questionFloat(
        "Digite o valor da comissão de venda: "
      );
