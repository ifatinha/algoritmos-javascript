...
function criarAgenda(pacientes) {
  ...
  while (eValido === null) {
    stringData = scanner.question(
      "Data inválida! Digite uma data no formato (dd/mm/aaaa): "
    );
    eValido = stringData.match(FormatadorData.paternData());
  }
  ...
  while (eValido === null) {
    stringHora = scanner.question(
      "Formato inválido! Digite um horario no formato (hh:mm): "
    );
    eValido = stringHora.match(FormatadorData.paternHora());
  }
  ...
  while (index === -1) {
    codigoPaciente = scanner.questionInt(
      "Paciente não encontrado! Informe o codigo do paciente novamente: "
    );
...
function agendarPaciente(pacientes, agenda) {
  ...
  if (jaExiste === false) {
    console.log(
      "Foi criado um agendamento com os seguintes dados: \n" +
        agendaPaciente.toString()
    );
    agenda.push(agendaPaciente);
  } else {
    console.log(
      "Erro! Já existe uma agenda para o dia " +
        agendaPaciente.data +
        " às " +
        agendaPaciente.hora
    );
