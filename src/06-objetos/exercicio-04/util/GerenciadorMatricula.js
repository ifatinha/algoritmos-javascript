...
function verificarMatricula(matriculas, matricula) {
  return matriculas.find(function (mat) {
    return (
      mat.aluno.codigo === matricula.aluno.codigo &&
  ...
  if (aluno !== undefined) {
    ...
    if (disciplina !== undefined) {
      ...
      if (verificarMatricula(matriculas, matricula) === undefined) {
        ...
        console.log(
          "Erro ao efetuar matricula! Já existe uma mátricula com esses dados!"
        );
      }
    } else {
...
function inserirNotas() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    while (nota < 0 && nota > 10) {
      nota = scanner.questionFloat(
        "Nota inválida! Informe um numero entre 0 e 10: "
      );
    }
...
function visualizarBoletim(matriculas) {
  ...
  let botetim = matriculas.filter(function (matricula) {
    return (
      matricula.aluno.codigo === alunoCod && matricula.anoLetivo === anoLetivo
    );
  });
