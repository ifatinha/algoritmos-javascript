...
function cadastrarFuncionario(funcionarios) {
  ...
  if (buscarFuncionario(funcionarios, funcionario) === undefined) {
    ...
    while (resposta === 1) {
      if (resposta === 1) {
        let dependente =
          gerenciadorDependentes.cadastrarDependente(funcionario);
    ...
    console.log(
      "Já existe um funcionário cadastrado com a matricula informada!"
    );
  }
    ...
    funcionarios.forEach(function (funcionario) {
      console.log(
        funcionario.matricula +
          ...
          funcionario.nome +
          "(" +
          funcionario.dependentes.length +
          " dependente(s))\n" +
          "Bônus: R$" +
          funcionario.bonus +
          "\n"
...
function alterarFuncionario(funcionarios) {
  ...
  if (index !== -1) {
    ...
    funcionarios[index].toString();
    funcionarios[index].salario = scanner.questionFloat(
