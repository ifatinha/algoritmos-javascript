...
function menu() {
    return scanner.questionInt("Escolha uma das opcoes abaixo\n" +
...
function cadastrarAniversariante(aniversariantes) {
    ...
    if (aniversariantes.length < 14) {
        if (jaExiste === -1) {
            ...
            console.log("Aniversariante cadastrado com os dados abaixo: \n" +
                aniversariante.toString());
        } else {
            console.log("Aniversariante não cadastrado! Já existe uma pessoa cadastrada com esse nome!");
        }

    } else {
        console.log("Aniversariante não cadastrado! Você já atingiu o numero maximo de cadastros!");
    }
...
function excluirAniversariante(aniversariantes) {
    let nome = scanner.question("Digite o nome do aniversariante que voce quer excluir: ");
    let index = buscarAniversarianteNome(aniversariantes, nome);

    if (index !== -1) {
        let opcao = scanner.questionInt("Deseja excluir o aniversariante \n" +
            aniversariantes[index].toString() + "? (1 - Sim/2 - Não): ");
...
function alterarDados(aniversariantes) {
    let nome = scanner.question("Digite o nome do aniversariante que voce quer alterar: ");
    let index = buscarAniversarianteNome(aniversariantes, nome);
...
export default {
    menu, cadastrarAniversariante, excluirAniversariante, alterarDados,
    consultarAniversariantesData, consultarAniversariantesMes, consultarAniversariantesLetra,
    ordenarAgendaNome, ordenarAgendaMes
}