...
function criarFuncionario() {
  ...
  let matricula =
    cargo.categoria + "-" + (Math.floor(Math.random() * 8999) + 1000);
  let nome =
    lastNames.generateRandomLastName() + ", " + firstNames.generateRandomName();
...
function funcionariosCategoria(funcionarios) {
  let categoria = scanner.question(
    "Digite a categoria que voce quer consultar (A, B, C, D, E): "
  );
  ...
  if (arr.length > 0) {
    console.log(
      "Lista de funcionarios que pertecem a categoria " +
        categoria.toUpperCase()
    );
    listarFuncionarios(arr);
    console.log(
      "\nValor Total de Salarios da Categoria: R$" +
        totalSalarioCategoria(arr).toFixed(2)
    );
  } else {
    console.log(
      "Não existe nenhum funcionario na categoria " + categoria.toUpperCase()
    );
  }
}
