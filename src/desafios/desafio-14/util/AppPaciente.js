...
function criarPaciente() {
    ...
    let nomeCompleto = Names.generateRandomName() + " " + LastNames.generateRandomLastName();
...
function cadastrarNovoPaciente(pacientes) {
    for (let i = 0; i < 10; i++) {
        ...
        if (jaExiste === false) {
            console.log("Um paciente com os dados " +
                paciente.toString() + " foi cadastrado!");
            pacientes.push(paciente);
        } else {
            console.log("Erro! Já existe um paciente cadastrado com o código " + paciente.codigo);
        }
    }
    return pacientes;
}
...
export default {
    cadastrarNovoPaciente, listarPacientes, alterarDadosPaciente,
    buscarPacienteIndex
}