  ...
  if (produtos.length < 6) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      while (qtdEstoque < qtdMinima) {
        console.log(
          "Quantidade não pode ser menor que a quantidade minima de " +
            qtdMinima
        );
      ...
      while (valorUnitario <= 0) {
        valorUnitario = scanner.questionFloat(
          "Preço não pode ser menor que 0! Digite novamente: "
        );
      }
...
function atualizarEstoque(produtos) {
  ...
  if (index !== -1) {
    ...
    let qtd = scanner.questionInt(
      "Digite a quantidade que você quer retirar do estoque: "
    );

    while (qtd > produtos[index].qtdEstoque) {
      qtd = scanner.questionInt(
    ...
    console.log(
      "Produto atualizado! A nova quantidade do estoque é " +
        produtos[index].qtdEstoque
    );
  } else {
