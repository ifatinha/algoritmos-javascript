...
function jogo(palavraSorteada) {
    ...
    for (let i = 7; i > 0; i--) {
        ...
        while (jaExiste === true) {
            letra = scanner.question("Você já jogou essa letra! Digite outra letra: ").toLowerCase();
            jaExiste = verificarLetra(letrasJogadas, letra);
        }

        letrasJogadas.push(letra);
        palavraEscondida = verificarPalpite(letra, palavraSorteada.palavra, palavraEscondida);

        console.log("Letras jogadas " + letrasJogadas.join(" "));
        console.log("Palavra escondida: " + palavraEscondida.join(" "));

        let chute = scanner.questionInt("Você ja sabe a palavra? (1 - Sim/2 - Não): ");
        
        while (chute !== 1 && chute !== 2) {
            chute = scanner.questionInt("Resposta invalida! Escolha (1 - Sim/2 - Não): ");
        }
