...
function geradorMatricula() {
    let letra = ["A", "B", "C", "D", "E"];
    let matricula = (letra[Math.floor((Math.random() * 5) + 0)]) + "-" +
        ...
        while (contador < 20) {
            ...
            if (contador < 10) {
                ...
                if (eMenor) {
                    ...
                    console.log("O aluno " + cursos[i].alunos[cursos[i].alunos.length - 1].toString() +
                        " foi removido do sistema porque " +
                        " tem nota inferior ao aluno " + aluno.toString());
                    cursos[i].alunos.pop();
                    cursos[i].alunos.push(aluno);
                    cursos[i].alunos = ordenarVetor(cursos[i].alunos);
                }
